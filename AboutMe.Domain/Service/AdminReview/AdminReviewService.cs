...
namespace AboutMe.Domain.Service.AdminReview
{
    public class AdminReviewService : IAdminReviewService
    {
         ...
         * 신- 상품리뷰조회
         */
        public Tuple<List<SP_ADMIN_REVIEW_FREE_SEL_Result>, int> ReviewFreeList(AdminReviewRouteParam p)
        {
            List<SP_ADMIN_REVIEW_FREE_SEL_Result> list = new List<SP_ADMIN_REVIEW_FREE_SEL_Result>();
            ObjectParameter total = new ObjectParameter("TOTAL", typeof(int));
            ...
            using (AdminReviewEntities context = new AdminReviewEntities())
            {
                list = context.SP_ADMIN_REVIEW_FREE_SEL(p.PAGE, 10, p.SEARCH_KEY, p.SEARCH_VALUE, p.MEDIA_GBN_W, p.MEDIA_GBN_M, start_dt, end_dt, p.IS_PHOTO_Y, p.IS_PHOTO_N, p.IS_DISPLAY_Y, p.IS_DISPLAY_N, total).ToList();
            }


            return new Tuple<List<SP_ADMIN_REVIEW_FREE_SEL_Result>, int>(list, Convert.ToInt32(total.Value)); ;
         ...
         * 상품리뷰 정보
         */
        public Tuple<SP_ADMIN_REVIEW_FREE_INFO_Result, string, string> ReviewFreeInfo(int? idx)
        {
            ObjectParameter retNum = new ObjectParameter("RET_NUM", typeof(string));
            ObjectParameter retMsg = new ObjectParameter("RET_MESSAGE", typeof(string));

            SP_ADMIN_REVIEW_FREE_INFO_Result info = new SP_ADMIN_REVIEW_FREE_INFO_Result();
            using (AdminReviewEntities context = new AdminReviewEntities())
            {
                info = context.SP_ADMIN_REVIEW_FREE_INFO(idx, retNum, retMsg).SingleOrDefault();
            }

            return new Tuple<SP_ADMIN_REVIEW_FREE_INFO_Result, string, string>(info, retNum.Value.ToString(), retMsg.Value.ToString());
        }

        /**
         * 상품리뷰  수정
         */
        public Tuple<string, string> ReviewFreeUpdate(AdminReviewInputViewModel p)
        {
            ObjectParameter retNum = new ObjectParameter("RET_NUM", typeof(string));
            ObjectParameter retMsg = new ObjectParameter("RET_MESSAGE", typeof(string));
            ...
            using (AdminReviewEntities context = new AdminReviewEntities())
            {
                context.SP_ADMIN_REVIEW_FREE_UPDATE(p.IDX, p.IS_BEST, p.IS_DISPLAY, p.IS_MOST_CNT, retNum, retMsg);
            }
